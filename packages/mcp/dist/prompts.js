...
function setupPromptBody(stack) {
...
export function registerPrompts(server) {
    server.prompt('setup_prodverdict', 'Guide for setting up ProdVerdict in a repository (prodverdict.yml + CI + optional MCP).', {
        stack: z
            .enum(STACKS)
            .optional()
    ...
    server.prompt('verify_before_pr', 'Run ProdVerdict doctor and all contract checks before opening a pull request.', {}, async () => ({
        messages: [
            {
                role: 'user',
                content: {
