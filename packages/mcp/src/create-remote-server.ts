...
import type { RemoteMcpAuth, RemoteMcpDeps } from './remote-types.js';
import {
...
function requirePro(auth: RemoteMcpAuth | null) {
  if (!auth) {
...
function requireGitHub(auth: RemoteMcpAuth) {
...
export function createRemoteMcpServer(
  ...
  server.tool(
    'validate_config',
    'Parse and validate prodverdict.yml without running checks. Free tier — YAML upload or GitHub file read.',
    {
    ...
    async ({ configYaml, configPath, owner, repo, ref }) => {
      try {
        ...
        if (owner && repo) {
          const auth = getAuth();
          const files = await deps.fetchRepoFiles(
            auth ?? { projectId: '', isPro: false, githubInstallationId: null },
            { owner, repo, ref },
          );
  ...
  server.tool(
    ...
    'Run config contract using GitHub App repo read (source + .env.example). ProdVerdict Cloud (Pro). No billing secrets on cloud.',
    {
      configPath: configPathSchema,
      ...repoSchema,
    },
  ...
  server.tool(
    ...
    'Run migration contract using GitHub App SQL reads. ProdVerdict Cloud (Pro). No billing secrets on cloud.',
    {
      configPath: configPathSchema,
      ...repoSchema,
    },
  ...
  server.tool(
    ...
    'Run config, migration, boundary, and webhook contracts in one call via GitHub App repo read. ProdVerdict Cloud (Pro). No billing secrets on cloud.',
    {
      configPath: configPathSchema,
      ...repoSchema,
    },
  ...
  server.tool(
    'get_recent_runs',
    'List recent contract runs uploaded to the ProdVerdict dashboard. ProdVerdict Cloud (Pro). Requires API key.',
    {
      limit: z
        .number()
        .int()
