...
export async function maybeNotifySlack(result: CheckResult, webhookUrl: string): Promise<void> {
  ...
  const text =
    `ProdVerdict ${result.contract} check: *${result.verdict.toUpperCase()}* (${result.findings.length} finding(s))\n` +
