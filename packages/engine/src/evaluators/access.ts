...
async function evaluateAccessSubscriptions(
  cfg: AccessContractConfig,
  billing: StripeReader,
  database: DatabaseReader,
): Promise<Finding[]> {
...
async function evaluateAccessEntitlements(
  cfg: Extract<AccessContractConfig, { source_of_truth: 'stripe_entitlements' }>,
  entitlements: EntitlementsReader,
  database: DatabaseReader,
): Promise<Finding[]> {
    ...
    if (!grants || grants.length === 0) {
      ...
      if (user.hasPaidAccess) {
        findings.push({
          ...
          entity: `user:${user.id}`,
          message:
            `User has has_paid_access=true but no active Stripe Entitlements grant for customer "${user.stripeCustomerId}". ` +
      ...
      for (const grant of grants) {
        ...
        if (expectedPlan === undefined) {
          ...
          findings.push({
            ...
            entity: `user:${user.id}`,
            message:
              `User plan is "${user.plan}" but active Stripe Entitlements feature key "${planKey}" maps to plan "${expectedPlan}".`,
...
function groupByCustomerId(subscriptions: StripeSubscription[]): Map<string, StripeSubscription[]> {
