...
function check(name: string, status: DoctorCheckStatus, message: string): DoctorCheck {
  return { name, status, message };
}

function envSet(env: Record<string, string | undefined>, key: string): boolean {
  const v = env[key];
  return v !== undefined && v !== '';
}

function collectAccessEnvVars(accessCfg: AccessContractConfig): string[] {
...
async function pingDatabase(url: string): Promise<void> {
...
async function pingStripe(secretKeyEnv: string, env: Record<string, string | undefined>): Promise<void> {
  const key = env[secretKeyEnv];
  if (!key) throw new Error(`Missing ${secretKeyEnv}`);
...
function validateConfigChecks(cfg: ProdVerdictConfig): DoctorCheck[] {
  return [
    check(
      'config:valid',
      'pass',
      `prodverdict.yml is valid — ${cfg.contracts.length} contract(s) defined.`,
    ),
  ];
}
...
function envVarChecks(
  ...
  return vars.map((name) => {
    ...
    return check(
      `${prefix}:env:${name}`,
      'fail',
      `Missing or empty env var "${name}". Export it before running live checks.`,
    );
  });
}
...
export async function runDoctor(opts: RunDoctorOptions): Promise<DoctorResult> {
  ...
  let cfg: ProdVerdictConfig;
  try {
    cfg = parseConfigFile(resolve(opts.configPath));
    checks.push(...validateConfigChecks(cfg));
  } catch (err) {
    checks.push(
      check('config:valid', 'fail', `prodverdict.yml is invalid: ${String(err)}`),
    );
      ...
      if (!opts.skipConnectivity) {
        ...
        if (dbUrl) {
          try {
            ...
            checks.push(
              check('access:database_ping', 'fail', `Database connection failed: ${String(err)}`),
            );
          }
        } else {
          checks.push(
            check(
              'access:database_ping',
              'skip',
              `Skipped — ${contract.database.url_env} not set.`,
            ),
          );
        }
        ...
        if (contract.source_of_truth === 'paddle') {
          if (envSet(env, contract.paddle.api_key_env)) {
            try {
              ...
              checks.push(
                check('access:paddle_ping', 'fail', `Paddle API failed: ${String(err)}`),
              );
            }
          } else {
            checks.push(
              check(
                'access:paddle_ping',
                'skip',
                `Skipped — ${contract.paddle.api_key_env} not set.`,
              ),
            );
          }
          ...
          if (envSet(env, contract.entitlements.secret_env)) {
            try {
              ...
              checks.push(
                check(
                  'access:stripe_entitlements_ping',
                  'pass',
                  'Stripe Entitlements API connection succeeded.',
                ),
              );
            } catch (err) {
              checks.push(
                check(
                  'access:stripe_entitlements_ping',
                  'fail',
                  `Stripe Entitlements API failed: ${String(err)}`,
                ),
              );
            }
          } else {
            checks.push(
              check(
                'access:stripe_entitlements_ping',
                'skip',
                `Skipped — ${contract.entitlements.secret_env} not set.`,
              ),
            );
          }
          ...
          try {
            ...
            checks.push(
              check('access:stripe_ping', 'fail', `Stripe API failed: ${String(err)}`),
            );
          }
        } else {
          checks.push(
            check(
              'access:stripe_ping',
              'skip',
              `Skipped — ${contract.stripe.secret_env} not set.`,
            ),
          );
        }
      }
        ...
        if (existsSync(examplePath)) {
          checks.push(
            check(
              'config:env_example',
              'pass',
              `${contract.env_example_file} exists.`,
            ),
          );
        } else {
          checks.push(
            check(
              'config:env_example',
              'fail',
              `${contract.env_example_file} not found at ${examplePath}.`,
            ),
          );
        }
      }
    ...
    if (contract.type === 'migration') {
      checks.push(
        check(
          'migration:paths',
          'pass',
          `Migration contract configured with ${contract.paths.length} path pattern(s).`,
        ),
      );
    }
      ...
      if (!opts.skipConnectivity) {
        ...
        if (dbUrl) {
          try {
            ...
            checks.push(
              check(
                'entitlements-migration:database_ping',
                'pass',
            ...
            checks.push(
              check(
                'entitlements-migration:database_ping',
                'fail',
                `Database connection failed: ${String(err)}`,
              ),
            );
          }
        }
        if (envSet(env, contract.entitlements.secret_env)) {
          try {
            ...
            checks.push(
              check(
                'entitlements-migration:stripe_ping',
                'pass',
                'Stripe Entitlements API connection succeeded.',
              ),
            );
