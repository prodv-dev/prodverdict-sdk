...
import type { EntitlementsReader, ActiveEntitlement } from './types.js';
 ...
 */
export function createLiveEntitlementsReader(secretKeyEnvVar: string): EntitlementsReader {
  const key = process.env[secretKeyEnvVar];
  if (!key) {
    throw makeConnectorError(
      `Missing required env var "${secretKeyEnvVar}" for Stripe Entitlements connector. ` +
  ...
  return {
    async listActiveEntitlements(): Promise<ActiveEntitlement[]> {
      const results: ActiveEntitlement[] = [];
      ...
      for await (const customer of client.customers.list({ limit: 100 })) {
        ...
        for await (const active of client.entitlements.activeEntitlements.list({
          ...
          const feature =
            typeof active.feature === 'string' ? active.feature : active.feature?.id;
          if (!feature) continue;
