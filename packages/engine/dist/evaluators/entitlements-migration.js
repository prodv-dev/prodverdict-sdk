 ...
 */
export async function evaluateEntitlementsMigration(cfg, sources) {
        ...
        for (const user of users) {
            if (user.hasPaidAccess && !user.stripeCustomerId) {
                findings.push({
                    ...
                    severity: 'high',
                    entity: `user:${user.id}`,
                    message: `User has has_paid_access=true but no stripe_customer_id. This user cannot be migrated ` +
    ...
    for (const user of users) {
        ...
        if (user.hasPaidAccess && (!grants || grants.length === 0)) {
            ...
            findings.push({
                ...
                severity,
                entity: `user:${user.id}`,
                message: `User has has_paid_access=true but no active Stripe Entitlements grant for customer ` +
...
//# sourceMappingURL=entitlements-migration.js.map