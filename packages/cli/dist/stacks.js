...
const CONFIG_BLOCK_PADDLE_SITE = `  - type: config
    ...
    rules:
      ...
      - type: required
        name: PADDLE_API_KEY
`;
function stripeAccessBlock(usersTable, fix) {
    return `  - type: access
...
function paddleAccessBlock(usersTable, fix) {
    return `  - type: access
...
export const STACK_LABELS = Object.fromEntries(STACK_ORDER.map((id) => [id, STACK_META[id].label]));
export const STACK_SELECT_LABELS = Object.fromEntries(STACK_ORDER.map((id) => [id, STACK_META[id].selectLabel]));
export const STACK_HINTS = Object.fromEntries(STACK_ORDER.map((id) => [id, STACK_META[id].hint]));
export function isStackTemplate(value) {
    return value in STACK_META;
}
export function isPaddleStack(stack) {
    return STACK_META[stack].billing === 'paddle';
}
export function fixtureExampleDir(stack) {
...
function accessBlock(stack) {
    switch (stack) {
        ...
        case 'neon-stripe':
            return stripeAccessBlock('users', 'Sync has_paid_access from Stripe webhooks. Use Neon read-only role for DATABASE_URL.');
        case 'clerk-stripe':
            return stripeAccessBlock('users', 'Clerk handles auth; sync stripe_customer_id and has_paid_access from Stripe webhooks.');
    }
    return stripeAccessBlock('users', 'Sync has_paid_access from Stripe webhooks.');
}
export function buildProdverdictYaml(stack, options) {
