...
export function usersTableForStack(stack) {
    if (stack === 'supabase-stripe' || stack === 'supabase-paddle')
        return 'profiles';
    return 'users';
}
/**
 ...
 */
export async function runPostgresRoleHelper(stack) {
    const envVar = 'DATABASE_URL';
    const alreadySet = Boolean(process.env[envVar]);
    const table = usersTableForStack(stack);
    console.log();
    console.log(chalk.bold('Create a read-only Postgres role (run this in psql or your DB console):'));
    console.log();
    ...
    console.log();
    console.log(`  ${chalk.cyan('export DATABASE_URL=postgresql://prodverdict_readonly:strong_password@host:5432/yourdb')}`);
    if (alreadySet) {
        console.log();
...
//# sourceMappingURL=postgres-role-helper.js.map