...
import { discoverEnvVars, applyEnvVars, mergeMcpEnv, missingEnvVars, } from './env-discovery.js';
function shouldWrite(path, force) {
    if (!existsSync(path))
        return true;
    return force;
}
function installWorkflow(repoRoot, force) {
    const wfPath = resolve(repoRoot, '.github/workflows/prodverdict-hourly.yml');
    if (!shouldWrite(wfPath, force))
        return null;
...
export async function runSetupNonInteractive(opts = {}) {
    const repoRoot = resolve(opts.repoRoot ?? process.cwd());
    const { stack: resolvedStack } = await resolveInitStack(repoRoot, opts.stack);
    if (!isStackTemplate(resolvedStack)) {
        throw new Error(`Unknown stack "${resolvedStack}". Pass --stack explicitly. Run: prodverdict init --list-stacks`);
    }
    ...
    if (!opts.skipMcp) {
        ...
        if (shouldWrite(mcpPath, force) || opts.fromEnv !== false) {
            ...
            if (!filesWritten.includes(written))
                filesWritten.push(written);
