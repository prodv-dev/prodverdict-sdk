...
export function buildScheduledWorkflow(opts: ScheduledOptions): string {
  ...
  const cronLine =
    frequency === 'hourly' ? `cron: '0 * * * *'` : `cron: '0 6 * * *'`;
  ...
  if (includeSlack) withLines.push(SLACK_INPUT_LINE);
...
export function registerScheduledCommand(program: Command): void {
  program
    .command('scheduled')
    .description(
      'Print a recommended GitHub Actions workflow for scheduled drift detection. Access is a scheduled check, not a PR gate.',
    )
    .option(
      '-f, --frequency <freq>',
      'Schedule frequency: hourly (default) or daily',
      'hourly',
    )
    .option('--no-slack', 'Omit slack_webhook_url from the workflow')
    .option(
      '-c, --contract <contract>',
      'Contract to schedule: access (default) or all',
      'access',
    )
    ...
    .option(
      '-o, --output <path>',
      'Output file path when --install is used (default: .github/workflows/prodverdict-<freq>.yml)',
    )
    .action((options: ScheduledOptions) => {
      const freq = options.frequency ?? 'hourly';
      if (freq !== 'hourly' && freq !== 'daily') {
        process.stderr.write(
          'Error: --frequency must be "hourly" or "daily" (got "' + freq + '")\n',
        );
        process.exit(2);
      }
      ...
      if (options.install) {
        const outputPath = resolve(
          options.output ?? '.github/workflows/prodverdict-' + freq + '.yml',
        );
