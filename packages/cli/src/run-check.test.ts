...
describe('runCheck with fixtures', () => {
  ...
  it('returns fail exit code on warn when --strict', async () => {
    const { exitCode, result } = await runCheck({
      config: fixtureConfig,
      format: 'json',
      fixtures: true,
      strict: true,
    });
    expect(exitCode).toBe(result.verdict === 'warn' ? 1 : result.verdict === 'fail' ? 1 : 0);
  });
...
describe('runCheck error handling', () => {
  it('throws CONFIG_INVALID for a missing config file', async () => {
    await expect(
      runCheck({ config: '/nonexistent/path/prodverdict.yml', format: 'json', fixtures: true }),
    ).rejects.toMatchObject({ code: 'CONFIG_INVALID' });
  });

  it('throws for unknown contract type', async () => {
    await expect(
      runCheck({ config: fixtureConfig, contract: 'banana', format: 'json', fixtures: true }),
    ).rejects.toMatchObject({ code: 'CONFIG_INVALID' });
  });
});
