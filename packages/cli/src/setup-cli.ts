...
interface WizardState {
  ...
  cursorRuleInstalled: boolean;
  skillsInstalled: boolean;
  doctorOk: boolean;
  firstCheckRan: boolean;
}
...
export async function runSetup(opts: SetupOptions): Promise<number> {
  ...
  if (!isStackTemplate(stack)) {
    process.stderr.write(
      chalk.red('Could not resolve a stack template. Pass --stack explicitly.\n'),
    );
    return 2;
  }
  ...
  if (configAlreadyExists) {
    writeConfig = await promptYesNo(
      `  prodverdict.yml already exists at ${state.configPath}. Overwrite?`,
      false,
    );
  }
  if (writeConfig) {
  ...
  try {
    ...
    if (state.doctorOk) {
      ...
      try {
        ...
        if (checkExit !== 0 && checkExit !== 1) {
          console.log(
            chalk.yellow('  ⚠ First check could not run — fix the issues above and re-run setup.'),
          );
        }
      } catch (err) {
        console.log(
          chalk.yellow(
            '  ⚠ First check failed to run: ' +
    ...
    console.log(
      chalk.yellow(
        '  ⚠ Doctor could not run: ' + (err instanceof Error ? err.message : String(err)),
      ),
    );
  }
  ...
  if (!opts.skipWorkflow) {
    ...
    const installWf = await promptYesNo(
      '  Write a scheduled GitHub Actions workflow for hourly drift detection?',
      true,
    );
    if (installWf) {
      ...
      try {
        ...
        if (existsSync(wfPath)) {
          const overwrite = await promptYesNo(
            `  ${wfPath} already exists. Overwrite?`,
            false,
          );
          if (!overwrite) {
        ...
        if (state.workflowInstalled) {
          console.log(
            chalk.dim('  Commit it, then set these repo secrets: STRIPE_SECRET_KEY, DATABASE_URL, SLACK_WEBHOOK_URL'),
          );
        }
      } catch (err) {
        console.log(
          chalk.yellow(
            '  ⚠ Could not write workflow file: ' +
              (err instanceof Error ? err.message : String(err)),
          ),
        );
      }
  ...
  if (!opts.skipMcp) {
    console.log();
    const installMcp = await promptYesNo(
      '  Also write a Cursor MCP config so your AI agent can run checks locally?',
      true,
    );
    if (installMcp) {
      try {
  ...
  if (!opts.skipCursorRule) {
    const installRule = await promptYesNo(
      '  Also write a Cursor agent rule so your AI agent knows the contracts?',
      true,
    );
    if (installRule) {
      try {
  ...
  if (!opts.skipSkills) {
    const installSkills = await promptYesNo(
      '  Also copy ProdVerdict agent skills into .cursor/skills/ for your team?',
      true,
    );
    if (installSkills) {
      try {
...
async function resolveStack(explicit?: string): Promise<string> {
  if (explicit) {
    if (!isStackTemplate(explicit)) {
      throw new Error(
        `Unknown stack "${explicit}". Run: prodverdict init --list-stacks`,
      );
    }
    return explicit;
  }
  ...
  if (detected) {
    const confirm = await promptYesNo(
      `  Detected stack: ${STACK_META[stack as keyof typeof STACK_META]?.label ?? stack}. Continue with this?`,
      true,
    );
    if (confirm) return stack;
  }
  console.log(
    chalk.dim('  Pass --stack <template> to specify. Run: prodverdict init --list-stacks'),
  );
  return stack;
}
...
function printHeader(): void {
  ...
  console.log(
    chalk.dim('Interactive first-run wizard. ~5 minutes. Press Ctrl+C at any time to bail.'),
  );
  console.log(chalk.dim('For AI agents: npx prodverdict setup --yes --format agent --from-env'));
  console.log(chalk.dim(divider(60)));
}

function printNonInteractiveSummary(agent: import('@prodverdict/engine').AgentSetupOutput): void {
  console.log();
...
function printSummary(state: WizardState): void {
  ...
  console.log();
  console.log(`  ${chalk.dim('Stack:')}              ${STACK_META[state.stack as keyof typeof STACK_META]?.label ?? state.stack}`);
...
export function registerSetupCommand(program: Command): void {
  program
    .command('setup')
    .description(
      'First-run setup: detect stack, credentials, config, scheduled workflow, Cursor MCP. Use --yes for non-interactive AI bootstrap.',
    )
    ...
    .action(async (options: SetupOptions & { fromEnv?: boolean }) => {
      try {
        ...
        if (options.format === 'agent') {
          process.stdout.write(
            JSON.stringify({ error: msg, schemaVersion: '1' }, null, 2) + '\n',
          );
        } else {
